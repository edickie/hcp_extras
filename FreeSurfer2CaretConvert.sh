...
FreeSurfer2CaretConvert.sh

Usage: FreeSurfer2CaretConvert.sh --FSpath=<path> --HCPpath=<path> --subject=<subject>
...
# ------------------------------------------------------------------------------
log_SetToolName "FreeSurfer2CaretConvert.sh"
...
${FSLDIR}/bin/applywarp --rel --interp=spline -i ${T1wFolder}/${T1wImage} -r ${T1wTemplate2mm} -w ${AtlasTransform}_warp -o ${AtlasSpaceFolder}/${T1wImage}
...
for Image in wmparc aparc.a2009s+aseg aparc+aseg ; do
  if [ -e "$FreeSurferFolder"/mri/"$Image".mgz ] ; then
    #mri_convert -rt nearest -rl "$T1wFolder"/"$T1wImage" "$FreeSurferFolder"/mri/"$Image".mgz "$T1wFolder"/"$Image"_1mm.nii.gz
    mri_convert "$FreeSurferFolder"/mri/"$Image".mgz "$T1wFolder"/"$Image"_1mm.nii.gz
...
for Hemisphere in L R ; do
  ...
  for Surface in white pial ; do
    ...
    #${CARET7DIR}/wb_command -surface-apply-affine "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii ${T1wFolder}/xfms/acpc.mat "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii -flirt "$T1wFolder"/${T1w_nonacpc}.nii.gz ${T1wTemplate}
    ...
    ${CARET7DIR}/wb_command -surface-apply-warpfield "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii "$InverseAtlasTransform"_warp.nii.gz "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii -fnirt "$AtlasTransform"_warp.nii.gz
